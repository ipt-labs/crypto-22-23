...
def bigram_frequency(text, mode): #filter with space if mode == 1
    ...
    for i in range(0, len(text) - 1):
        ...
        answer[bigram] = (answer.setdefault(bigram, 0) + 1)
    for i in answer:
        answer[i] = answer[i]/len(text)
    return answer
...
print(h_count(letter_frequency(text, 1))) # ентропія літер у тексті
print(h_count(bigram_frequency(text, 1))) # ентропія біграм у тексті, які перетинаються
print(h_count(bigram_frequency_2(text, 1))) # ентропія біграм у тексті, які не перетинаються з кроком 2
...
print(h_count(letter_frequency(text, 2))) # ентропія літер у тексті
print(h_count(bigram_frequency(text, 2))) # ентропія біграм у тексті, які перетинаються
print(h_count(bigram_frequency_2(text, 2))) # ентропія біграм у тексті, які не перетинаються з кроком 2
